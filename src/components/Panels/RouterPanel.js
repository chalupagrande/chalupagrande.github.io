...
export function RouterPanel(props) {
  const { routes, title, defaultPosition, size, resizable, background, padding, ...rest } = props
  ...
  return (
    <Panel
      ...
      isRouterPanel={true}
    >
      <Router basepath={`/${title.toLowerCase()}`} primary={false}>
        <LocationTracker path="/*" />
        {routes.map((RouteComponent, index) => {
          return (
            <RouteComponent.Component key={index} path={RouteComponent.path || "/"} {...rest} />
          )
        })}
        <NotFound default />
      </Router>
